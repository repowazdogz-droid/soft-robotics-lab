...
export default function AssumptionsPage() {
  const config = roomConfig.assumptions;
  const { workspace, addItem, removeItem } = useWorkspace();
  const [inputValue, setInputValue] = useState('');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (inputValue.trim()) {
      addItem('assumptions', inputValue);
      setInputValue('');
    }
  };
  ...
  const items = workspace.assumptions;
  const isStarterItem = (id: string) => id.startsWith('starter-');
      ...
      >
        {items.length === 0 ? (
          <p className="site-text-sm" style={{ color: '#737373' }}>
            Assumptions list will appear here
          </p>
        ) : (
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {items.map((item) => {
              const isStarter = isStarterItem(item.id);
              return (
                <li
                  key={item.id}
                  style={{
                    ...
                    border: isStarter ? '1px dashed #d4d4d4' : '1px solid #e5e5e5',
                ...
                >
                  ...
                  <button
                    onClick={() => removeItem('assumptions', item.id)}
                    style={{
                      background: 'none',
                      border: 'none',
                      color: '#737373',
