...
enum InspectorCopy {
    ...
    static func assumptionMetadata(for assumptionId: String, preset: AssumptionPreset? = nil, packContext: PackContext? = nil) -> (title: String, meaning: String)? {
        ...
        guard let fallback = defaultAssumptions[assumptionId] else { return nil }
        
        // Priority: pack override > preset > fallback
        ...
        let title = packTitle ?? preset?.assumptionTitles[assumptionId] ?? fallback.title
        let meaning = packMeaning ?? preset?.assumptionMeanings?[assumptionId] ?? fallback.meaning
    ...
    static func conflictMetadata(for conflictId: String, packContext: PackContext? = nil) -> (title: String, meaning: String)? {
        ...
        guard let fallback = defaultConflicts[conflictId] else { return nil }
        
        // Priority: pack override > fallback
        ...
        let title = packTitle ?? fallback.title
        let meaning = packMeaning ?? fallback.meaning
