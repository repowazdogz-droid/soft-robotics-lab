...
struct LinkLayer: LayerProtocol {
    ...
    private static let linkSpecs: [AssumptionLinkSpec] = [
        AssumptionLinkSpec(
            id: "L1",
            fromNodeId: "N1",
            toNodeId: "N2",
...
class LinkMetadataStore {
    ...
    private var links: [String: AssumptionLinkSpec] = [:]
    
    private init() {}
    
    func store(_ spec: AssumptionLinkSpec) {
        links[spec.id] = spec
    }
    
    func get(_ id: String) -> AssumptionLinkSpec? {
        return links[id]
    }
}
