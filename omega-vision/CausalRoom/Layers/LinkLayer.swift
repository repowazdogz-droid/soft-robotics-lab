...
struct LinkLayer: LayerProtocol {
    ...
    private static let linkSpecs: [CausalLinkSpec] = [
        CausalLinkSpec(
            id: "L1",
            fromNodeId: "N1",
            toNodeId: "N2",
...
class LinkMetadataStore {
    ...
    private var links: [String: CausalLinkSpec] = [:]
    
    private init() {}
    
    func store(_ spec: CausalLinkSpec) {
        links[spec.id] = spec
    }
    
    func get(_ id: String) -> CausalLinkSpec? {
        return links[id]
    }
}
