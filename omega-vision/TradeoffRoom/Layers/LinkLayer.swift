...
struct LinkLayer: LayerProtocol {
    ...
    private static let linkSpecs: [TradeoffLinkSpec] = [
        TradeoffLinkSpec(
            id: "L1",
            fromNodeId: "N1",
            toNodeId: "N2",
...
class LinkMetadataStore {
    ...
    private var links: [String: TradeoffLinkSpec] = [:]
    
    private init() {}
    
    func store(_ spec: TradeoffLinkSpec) {
        links[spec.id] = spec
    }
    
    func get(_ id: String) -> TradeoffLinkSpec? {
        return links[id]
    }
}
