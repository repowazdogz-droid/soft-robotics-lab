...
struct TradeoffsLayer: LayerProtocol {
    ...
    init() {
        // Create "Tradeoffs" group entity
        let tradeoffsGroup = Entity()
        tradeoffsGroup.name = "Tradeoffs"
        
        // Tradeoff material: soft, neutral, selectable
        let tradeoffMat = SimpleMaterial(
        ...
        // T1-T6 tradeoffs (placeholder IDs, will be replaced with actual tradeoff geometry)
        let tradeoffDefs: [(String, Float)] = [
            ("A1", startAngle),
        ...
        rootEntity.addChild(tradeoffsGroup)
    }
}
