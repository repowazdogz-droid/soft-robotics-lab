...
Timeouts and graceful degradation: memory/substrate optional; LLM timeout 30s.
"""
...
# Timeouts (seconds)
_INIT_MODELS_TIMEOUT = 3
_MEMORY_CONTEXT_TIMEOUT = 2
_LLM_TIMEOUT = 30
_EXERCISE_TIMEOUT = 5
_executor = ThreadPoolExecutor(max_workers=4, thread_name_prefix="tutor")
...
class TutorEngine:
    ...
    def __init__(self, api_key: str = None):
        ...
        try:
            ...
            future = _executor.submit(self.local_client.models.list)
            models = future.result(timeout=_INIT_MODELS_TIMEOUT)
            if models.data:
    ...
    def teach(self, question: str, level: str = "adult") -> TeachingResponse:
        ...
        Memory/substrate calls are optional with timeouts; record_learning runs in background.
        """
        user_prompt = adapt_prompt(question, level)
        full_system = f"{TUTOR_SYSTEM_PROMPT}\n\nCurrent level instruction: {get_level_prompt(level)}"
        if memory:
            try:
                future = _executor.submit(memory.get_related_learning, question, 3)
                related = future.result(timeout=_MEMORY_CONTEXT_TIMEOUT)
                if related:
                    context_lines = []
                    for r in related:
                        topic = r.get("topic", "")
        ...
        if _ex_level and len(question.split()) > 3:
            try:
                future = _executor.submit(self.exercise_gen.generate, question[:200], _ex_level)
                ex = future.result(timeout=_EXERCISE_TIMEOUT)
        ...
        if memory:
            ...
            payload = (topic_name, level, parsed.explanation + "\n\n" + parsed.example)

            def _record():
                try:
                    memory.record_learning(*payload)
                except Exception:
                    pass
